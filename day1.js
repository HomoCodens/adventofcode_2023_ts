...
const parseSpelledNumber = (str, reversed) => {
    console.log(`tryna parse an ${str}`)
    const mappa = {
        one: 1,
        two: 2,
        three: 3,
    ...
    for(let [key, value] of Object.entries(mappa)) {
        if((!reversed && str.endsWith(key)) || (reversed && str.startsWith(key))) {
            return value
        }
    }
...
const getFirstDigit = (line, digitsOnly = true, reversed = false) => {
    if(digitsOnly) {
        let digits = /[0-9]/
        let digitsInLine = line.split('').filter(ltr => digits.test(ltr))
        return Number.parseInt(digitsInLine[0])
    } else {
        let arr = line.split('')
        //console.log('eating a')
        //console.log(arr)
        let spelledBuffer = []
        let indexOfSpelledNumber = 0
        for(var ltr of arr) {
            //console.log(`trying a ${ltr}`)
            const parsed = Number.parseInt(ltr)
            if(!Number.isNaN(parsed)) {
                //console.log('mmh, delicious number')
                return parsed
            } else {
                //console.log('whop, not a number')
                spelledBuffer.push(ltr)
                //console.log(spelledBuffer)
                const parsedSpelled = parseSpelledNumber(reversed ? [...spelledBuffer].reverse().join('') : spelledBuffer.join(''), reversed)
                if(!Number.isNaN(parsedSpelled)) {
                    //console.log('aHA')
                    //console.log(parsedSpelled)
                    return parsedSpelled
                }
            }
        }
    }
...
const getLastDigit = (line, digitsOnly = true) => {
    return getFirstDigit(line.split('').reverse().join(''), digitsOnly, true)
}

const eatNumbers = (arr) => {
    //console.log('eating a')
    //console.log(arr)
    const outBuffer = []
    let spelledBuffer = ''
    ...
    for(var ltr of arr) {
        ...
        if(!Number.isNaN(parsed)) {
            //console.log('mmh, delicious number')
            outBuffer.push(parsed)
            //console.log(outBuffer)
            spelledBuffer = ''
            indexOfSpelledNumber++
        } else {
            //console.log('whop, not a number')
            spelledBuffer += ltr
            //console.log(spelledBuffer)
            const parsedSpelled = parseSpelledNumber(spelledBuffer)
            if(!Number.isNaN(parsedSpelled)) {
                //console.log('aHA')
                //console.log(parsedSpelled)
                spelledBuffer = ''
                outBuffer.splice(indexOfSpelledNumber, 0, parsedSpelled)
                indexOfSpelledNumber = outBuffer.length + 1
                //console.log(outBuffer)
            }
        }
    }
...
const doIt = (input, part2 = false) => {
    const out = input.split('\n')
         .map((line) => {
            return 10*getFirstDigit(line, !part2) + getLastDigit(line, !part2)
         })
    console.log(out)
    return out.reduce((a, b) => a + b)
}

//console.log(doIt(exampleInput))

const realInput = `eightqrssm9httwogqshfxninepnfrppfzhsc
...
console.log(doIt(exampleInput2, true))
console.log(doIt(realInput, true))
//eatNumbers('2oneight'.split(''))
//console.log(getFirstDigit('bptwone4sixzzppg', false))
//console.log(getLastDigit('bptwone4sixzzppg', false))